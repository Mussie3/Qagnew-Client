            ...
            <div className="flex justify-center w-full">
                <div className="">
                    <div className="w-full flex justify-center p-2">
                        <div>
                            <div className="w-full px-10 flex justify-center"><div className="flex justify-center"><a href={mainNews[0].link} target="_blank" rel="noopener noreferrer"><img src={mainNews[0].image} alt="news-image" className="rounded-[15px] w-full"/></a></div></div>
                            <div className="w-full">
                                <div className={`font-MaruBuri_Regular ${isSmall ? 'text-lg' : 'text-xl '} font-semibold py-2 px-4`}><a href={mainNews[0].link} target="_blank" rel="noopener noreferrer">{mainNews[0].title}</a></div>
                                <div className={`flex w-full font-MaruBuri_Regular  ${isSmall ? 'text-[10px]' : 'text-sm'} text-sm py-3 px-4`}>
                                    <div className="w-1/4">{mainNews[0].author}</div>
                                    <div className="w-1/4">{mainNews[0].date}</div>
                                </div>
                                <div className="px-4">
                                    <div className={`font-MaruBuri_Regular ${isSmall ? 'text-sm' : 'text-md'} py-1`}>{mainNews[0].par_1}</div>
                                    <div className={`font-MaruBuri_Regular ${isSmall ? 'text-sm' : 'text-md'} py-1`}>{mainNews[0].par_2}</div>
                                </div>
                            </div>
                        </div>
                    </div>
                    <div className="grid grid-row-5 w-full px-8">
                        {
                            picNews.map((data)=>
                            <div key={data.id} className="flex w-full border-b-[2px]">
                                <div className="w-1/3 p-2 flex justify-center"><a href={data.link} target="_blank" rel="noopener noreferrer"><img src={data.image} alt="news-img" className="rounded-[5px] w-[200px]"/></a></div>
                                <div className="w-2/3 p-2 h-full">
                                    <div className={`h-2/3 ${isSmall ? 'text-sm' : ''} font-MaruBuri_Regular font-semibold`}><a href={data.link} target="_blank" rel="noopener noreferrer">{data.title}</a></div>
                                    <div className="h-1/3 flex items-end w-full">
                                        <div className={`font-MaruBuri_Regular ${isSmall ? 'text-[10px]' : 'text-sm'} w-1/2`}>{data.author}</div>
                                        <div className={`font-MaruBuri_Regular ${isSmall ? 'text-[10px]' : 'text-sm'} w-1/2`}>{data.date}</div>
                                    </div>
                                </div>
                            </div>
                            )
