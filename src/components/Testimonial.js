...
const Testimonial = (props) => {
    ...
    return (
        <div className="h-screen w-screen realtive">
            {wasClicked && 
            <Overlay
                selected = {selected}  
                onClick={handleClose}
            ...
            />}
            <div className="h-[25%] flex items-end w-full justify-center"><div className="w-11/12 font-MaruBuri_Regular py-8 text-3xl font-semibold">강뉴는 여러분의 경험으로 채워갑니다.</div></div>
            <div className={`flex w-screen h-[60%] items-center ${isMedium ? 'px-1' : 'px-8'} py-4 relative`}>
            {
                testimonials.map((data)=>
                <div key={data.id} className={`h-full flex justify-center ${isMedium ? 'w-1/3' : 'w-1/4'}`}>
                    <Card className={`w-[350px]`}>
                        ...
                        <div className="px-6 h-3/6">
                            ...
                            <div className={`font-MaruBuri_Regular text-sm font-semibold py-1 px-2 leading-7`}>{data.summery}</div>
                        </div>
                        <div className="flex w-full items-end p-4 h-1/6">
                            <div className="w-1/2"><button id={data.id} onClick={handleClick} className={`font-MaruBuri_Regular ${isMedium ? 'text-[13px]' : 'text-sm'} px-4 font-semibold flex items-center`}>더 읽어보기 <span className="flex px-2"><Right className="fill-[#FFC60B] w-2 h-2"/><Right className="fill-[#FFC60B] w-2 h-2"/></span><span></span></button></div>
