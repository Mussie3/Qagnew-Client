...
const Partners = () => {
    return (
        <div className='h-screen w-screen'>
            <div className="w-screen justify-center h-1/4 flex items-end"><div className="text-3xl font-MaruBuri_Regular font-semibold p-8 w-11/12">강뉴는 다양한 고객/기관과 성장의 가치를 함께합니다.</div></div>
            <div className="flex justify-center w-screen">
                <div className="grid grid-cols-4 gap-4 w-4/5 h-3/4">
                    {
                    partners.map((data)=>
                        <div key={data.id} className="flex justify-center h-28 p-1">
                            <div className="p-2 h-20 w-28 flex items-center"><img src={data.logo} alt="partner-logo" className="w-24"/></div>
                        </div>
                    )   
                    }
                </div>
