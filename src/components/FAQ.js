...
const FAQ = (props) => {
    ...
    const [arrayValue, setArrayValue] = useState(faqData.filter(data => data.id < 5));
    const [disabled, setDisabled] = useState(false);

    const handleClicked = val =>{
        console.log(selected);
        if(val.currentTarget.id === 'next' && selected === 7){
            const result = faqData.filter(word => +word.id === +selected);
            setSelected(7);
            setDisabled(true);
            setAnswer(()=>{return result[0].answer});
        }else if (val.currentTarget.id === 'next' && selected !== 7)
        {
            const result = faqData.filter(word => +word.id === +selected+1);
            setSelected(prev=>{return prev+1});
            setAnswer(()=>{return result[0].answer});
        } else if (val.currentTarget.id === 'prev' && selected !== 0)
        {
            const result = faqData.filter(word => +word.id === +selected-1);
            setSelected(prev=>{return prev-1});
            setAnswer(()=>{return result[0].answer});
    ...
    // const handleClicked = val => {
    //     if(val.currentTarget.id === 'next') {
    //         setArrayValue(faqData.filter(data => data.id >= 5));
    //         const result = faqData.filter(word => +word.id === 5);
    //         setSelected(5);
    //         setDisabled(true)
    //         setAnswer(()=>{return result[0].answer});
    //     } if(val.currentTarget.id === 'prev') {
    //         setArrayValue(faqData.filter(data => data.id < 5));
    //         const result = faqData.filter(word => +word.id === 0);
    //         setSelected(0);
    //         setDisabled(false);
    //         setAnswer(()=>{return result[0].answer});
    //     }
    ...
    const handleSelected = value => {
        console.log(value);
        const result = faqData.filter(word => +word.id === +value);
    ...
    <div className="w-full">
        ...
        <div className="w-4/5 justify-center">
            <div className="flex pt-4 pb-2">
                <div className="px-2"><Left className={`w-4 h-4 ${selected === 0 ? 'fill-[#6e6e6e]  cursor-not-allowed' : 'fill-[#000]  cursor-pointer'} select-none`} onClick = {handleClicked} id="prev"/></div>
                <div className="px-2"><Right className={`w-4 h-4 ${selected === 7 ? 'fill-[#6e6e6e]  cursor-not-allowed' : 'fill-[#000]  cursor-pointer'} select-none`} onClick = {handleClicked} id="next"/></div>
            </div>
            <div className="w-full flex h-3/4 pt-1">
                ...
                <div className="flex w-3/5">
                    <div className={`${isMedium ? 'px-4' : 'px-16'}`}>
                        <div className="px-10"><img src={support} className="w-24 h-24"/></div>        
                        <div className={`w-full p-5 rounded-[5px] mb-2 font-MaruBuri_Regular select-none text-md leading-10`}>
                            {/* <Typewriter
                            options={{
                                strings: [`${answer}`],
                                autoStart: true,
